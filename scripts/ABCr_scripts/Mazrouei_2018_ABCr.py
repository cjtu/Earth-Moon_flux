### ABCr Sampling routine for crater age distribution properties; 
### Mazrouei et al. 2018.
...
def bootstrap_abcr(x, xmax, x_earth, N=100000):
	...
	# Given arcsin prior on break location, 
	# gamma is for case where intervals are symmetric; 
	# modify by weighting to intervals.
	N_left_1 =  gamma * break_loc / xmax
	N_right_1 = (1.0 - gamma) * (xmax-break_loc) / xmax
	N_650_1 = f_lt_thresh(gamma, break_loc, xmax, 650.0)

	N_samp_left = np.sum(x <= break_loc) + np.sum(x_earth <= break_loc)
	N_samp_right = np.sum(x >  break_loc) + np.sum(x_earth >  break_loc)
	N_samp_650 = x_earth.size + np.sum(x <= 650)
	
	N_needed = 10.0 * np.array([N_samp_left / N_left_1,
								N_samp_right / N_right_1,
								N_samp_650 / N_650_1])
	...
	if big_sample_size > 1e5:
		...
		D2 = (anderson_ksamp([x, sample_uniform])[0] + 
			  anderson_ksamp([x_earth, sample_uniform_earth])[0])
	...
	D = (anderson_ksamp([x, sample])[0] + 
	 	 anderson_ksamp([x_earth, sample_earth])[0])
	D2 = (anderson_ksamp([x, sample_uniform])[0] +
		  anderson_ksamp([x_earth, sample_uniform_earth])[0])
...
if __name__ == "__main__":
	...
	d_big = pd.read_csv('RockyCraters_lg10_forBill.csv')
	...
	d_Earth = pd.read_csv('terrestrial_650under_052617.csv')
	AGES_Earth = np.array(d_Earth['Age'])

	### Read crater age regression parameter PDF
	d_reg = pd.read_csv('crater_regression_parameter_pdf.csv', skipinitialspace=True)
	ok = np.where( d_reg['lnprob'] > np.percentile(d_reg['lnprob'], 0.1) )
	sample_regress_params = np.array([d_reg['a'].values[ok], d_reg['b'].values[ok]]).T
	...
	with open('ABCr_crater_rate_JOINT_MARGINALIZED_OVER_UNCERTAINTY.csv', 'w') as f:
		f.write('### Threshold D: %s\n'%(np.percentile(all_D, 0.01)))
		f.write('### Number of uniform cases with D<D_thresh: %s\n'%(np.sum(all_D2 <= np.percentile(all_D, 0.01))))
		f.write('### Number of broken cases with D<D_thresh: %s\n'%(np.sum(all_D <= np.percentile(all_D, 0.01))))
		f.write('### Total number of broken cases: %s\n'%(all_D.shape[0]))
		f.write('D_break,a,b,break,gamma\n')
		for i in ok[0]:
